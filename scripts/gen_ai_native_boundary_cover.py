#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""生成 AI 原生组织新文配图：banner(1216x832) + og(1200x630 jpg/webp)。
...
SLUG = "ai-native-org-capability-boundary-2026"
TITLE = "AI原生组织不是范式切换：是能力边界的重写"
CAT = "AI原生组织 · 架构重构"
...
def draw_flow(d, w, h, seed):
    """右侧流程化节点网络，象征能力边界流动"""
    import random
    rnd = random.Random(seed)
    nodes = []
    for _ in range(12):
        x = rnd.randint(int(w * 0.55), int(w * 0.92))
    ...
    for i, (x1, y1) in enumerate(nodes):
        for j, (x2, y2) in enumerate(nodes[i + 1:], i + 1):
            ...
            if d2 < (w * 0.18) ** 2:
                ...
                arrow_size = 4
                d.polygon([
                    (int(mid_x + arrow_size * math.cos(angle)),
                     int(mid_y + arrow_size * math.sin(angle))),
                    (int(mid_x + arrow_size * math.cos(angle + 2.5)),
                     int(mid_y + arrow_size * math.sin(angle + 2.5))),
                    (int(mid_x + arrow_size * math.cos(angle - 2.5)),
                     int(mid_y + arrow_size * math.sin(angle - 2.5)))
                ], fill=(255, 255, 255))
    # 节点
