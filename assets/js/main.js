...
function trackEvent(name,params){params=params||{};if(window.aihrTrack){try{window.aihrTrack(name,params);}catch(e){}}if(typeof gtag!=='undefined'){try{gtag('event',name,params);}catch(e){}}}try{
...
/* ===== 模块 10：站内二跳与转化路径分类（2026-09-21，P0-1/P0-2） =====
   目的：把「读者读完去了哪」变成可度量的分类事件，而不是只看跳出率。
   分类：/ask/ → copilot_open；/tools/ → assessment_start；/hub/ → hub_click；其余站内 → internal_link_click。
   带 data-track 的元素由 analytics-loader 的统一监听器采集，此处跳过，避免重复上报。 */
try{
(function(){
  function classify(href){
    if(/\/ask\/?(\?|$)/.test(href))return 'copilot_open';
    if(/\/tools\//.test(href))return 'assessment_start';
    if(/\/hub\//.test(href))return 'hub_click';
    return 'internal_link_click';
  }
  document.addEventListener('click',function(e){
    var a=e.target&&e.target.closest?e.target.closest('a'):null;
    if(!a)return;
    if(a.closest('[data-track]'))return;
    var href=a.getAttribute('href')||'';
    if(!href||href.charAt(0)==='#')return;
    var isInternal=href.charAt(0)==='/'||href.indexOf('aihrlab.online')>-1;
    if(!isInternal)return;
    if(typeof trackEvent==='function'){trackEvent(classify(href),{target:href,label:href,page_path:window.location.pathname});}
  });
})();
}catch(e){console.error('[AIHR main.js] module 10 init failed:', e);}
